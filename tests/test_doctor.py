...
class DoctorScriptTests(unittest.TestCase):
    def test_doctor_passes_with_expected_warnings(self) -> None:
        ...
        self.assertRegex(
            result.stdout,
            rf"INFO: agents={agent_count} teams={team_count} skills={skill_count} rules={rule_count} hooks={hook_count}",
        )
    ...
    def test_readme_counts_match_repo(self) -> None:
        ...
        self.assertIn(f"{team_count} reusable team manifests", readme)
        self.assertIn(f"{skill_count} reusable skills", readme)
        self.assertIn(
            f"agents={agent_count} teams={team_count} skills={skill_count} rules={rule_count} hooks={hook_count}",
            self._doctor_output(),
        )
